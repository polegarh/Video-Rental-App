...
final class InventorySet implements Inventory {
  ...
  Record checkOut(Video video) {
	...
	//if (!(_data.get(video) instanceof RecordObj)) throw new IllegalArgumentException();
	  
	//Record copy = (RecordObj) _data.get(video);	
	RecordObj r = (RecordObj)_data.get(video);
	if (r == null || r.numOut == r.numOwned) throw new IllegalArgumentException("Sorry not sorry, but you cant check out");
	r.numOut++; r.numRentals++;  
	return r;
  }
  ...
  Record checkIn(Video video) {
	...
	if (r == null || r.numOut == 0 ) throw new IllegalArgumentException("No Video Record"); 
	r.numOut--;
	return copy;
  }
